...
There are two builds: `jwtd` (all six targets) and a windows-only `jwtd-windows` whose settings are identical, so its `jwtd.exe` is byte-for-byte the same binary as in the windows `tar.gz`. The `tar.gz` archives carry archive id `jwtd`; the windows zips carry id `jwtd-zip` (fed by `jwtd-windows`). The separate id exists so `scoops.ids: [jwtd]` can pin Scoop to the `tar.gz` archive — GoReleaser's scoop pipe errors if it sees more than one windows archive. WinGet cannot consume `tar.gz`, which is the only reason the zips exist.
