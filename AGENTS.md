...
Artifacts cross the build/release job boundary as two separate uploads: `jwtd-release-assets` (everything published to the GitHub release) and `jwtd-manifests` (the cask). The release job downloads only the former, so a downstream manifest can never become a release asset.
